...
public class CrateAPI {
    ...
    public void getChange(Player player, PlayerInventory playerInventory, int number) {
        ...
        Enchantment efficiency = Enchantment.getEnchantment(Enchantment.ID_EFFICIENCY)
                .setLevel(1);
        Item diamondSword = Item.get(ItemIds.DIAMOND_SWORD);
        Item diamondPickaxe = Item.get(ItemIds.DIAMOND_PICKAXE);
        Item[] items = new Item[0];
        if (number >= 1) {
            if (number <= 10) {
                /*items = new Item[]{
                        Item.get(ItemIds.TNT, 0, 8),
                        Item.get(ItemIds.GOLDEN_APPLE, 0, 1),
                        Item.get(ItemIds.GOLDEN_APPLE_ENCHANTED, 0, 1),
                        Item.get(ItemIds.OBSIDIAN, 0, 12)
                };*/
            }
            if (number > 10) {
                if (number <= 20) {
                    /*items = new Item[]{
                            Item.get(ItemIds.TNT, 0, 5),
                            Item.get(ItemIds.GOLDEN_APPLE, 0, 3),
                            Item.get(ItemIds.GOLDEN_APPLE_ENCHANTED, 0, 1),
                            Item.get(ItemIds.OBSIDIAN, 0, 16)
                    };*/
                }
            }
            if (number > 20) {
